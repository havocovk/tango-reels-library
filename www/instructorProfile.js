...
// ✅ YENİDEN YAPILANDIRILDI: Eğitmenler listesi instructorsList.js'e taşındı.

import { dbUpdateInstructorProfile } from './tangoVeritabani.js';
import { uploadInstructorPhoto } from './storage.js';
...
export function openInstructorProfile(instructorId) {
    ...
    if (callSwitchViewFn) callSwitchViewFn('instructorProfile', { instructorId });
}
...
export function renderInstructorProfile(instructorId) {
    ...
    const photoHtml = instructor.photo_url
        ? `<img src="${escapeHtml(instructor.photo_url)}" alt="${escapeHtml(instructor.name)}" style="width:100px;height:100px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,0,127,0.5);box-shadow:0 0 20px rgba(255,0,127,0.3);" onerror="this.style.display='none';this.nextSibling.style.display='flex';">
    ...
    const videosHtml   = videos.length === 0
        ...
        : videos.map(v => {
            ...
            return `<div class="prof-video-row" data-vid-id="${v.id}" data-vid-platform="${platform}" data-vid-url="${escapeHtml(v.url || '')}" data-vid-drive="${escapeHtml(v.drive_url || '')}">
    ...
    const igLink = instructor.instagram_url ? `<a href="${escapeHtml(instructor.instagram_url)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;color:#e1306c;opacity:0.85;transition:opacity 0.15s;text-decoration:none;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.85'" title="Instagram"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg></a>` : '';
    const fbLink = instructor.facebook_url  ? `<a href="${escapeHtml(instructor.facebook_url)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;color:#1877f2;opacity:0.85;transition:opacity 0.15s;text-decoration:none;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.85'" title="Facebook"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg></a>` : '';
    const ytLink = instructor.youtube_url   ? `<a href="${escapeHtml(instructor.youtube_url)}" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;color:#ff0000;opacity:0.85;transition:opacity 0.15s;text-decoration:none;" onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.85'" title="YouTube"><svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg></a>` : '';
    const socialLinksHtml = (igLink || fbLink || ytLink) ? `<div style="display:flex;align-items:center;gap:10px;margin-top:8px;">${igLink}${fbLink}${ytLink}</div>` : '';

    // ── Ana HTML ─────────────────────────────────────────────
    container.innerHTML = `
        <style>
            .prof-avatar-fallback { width:100px;height:100px;border-radius:50%;background:linear-gradient(135deg,#ff007f,#00f0ff);display:flex;align-items:center;justify-content:center;font-size:2rem;font-weight:700;color:#0b0813;border:3px solid rgba(255,0,127,0.5);box-shadow:0 0 20px rgba(255,0,127,0.3);flex-shrink:0; }
            .prof-stat-box { background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.09);border-radius:14px;padding:14px 18px;text-align:center;min-width:80px;flex:1; }
            ...
            .prof-platform-chip { display:inline-block;padding:3px 9px;background:rgba(0,240,255,0.08);border:1px solid rgba(0,240,255,0.2);border-radius:999px;font-size:0.72rem;color:#94a3b8;margin:3px 2px; }
            .prof-tag-chip { display:inline-block;padding:4px 10px;background:rgba(255,0,127,0.08);border:1px solid rgba(255,0,127,0.2);border-radius:999px;font-size:0.76rem;color:#f1a1c0;margin:3px 2px;cursor:default; }
            .prof-tag-chip sup { font-size:0.6rem;opacity:0.6; }
            .prof-video-row { display:flex;align-items:center;gap:12px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);border-radius:12px;padding:9px 12px;margin-bottom:8px;transition:border-color 0.2s;cursor:default; }
            .prof-video-row:hover { border-color:rgba(255,0,127,0.25); }
            .prof-video-thumb { flex-shrink:0;width:48px;height:48px;border-radius:10px;background-size:cover;background-position:center;background-color:#1e293b; }
        ...
        <div style="display:flex;align-items:center;gap:22px;flex-wrap:wrap;background:rgba(255,255,255,0.03);border:1px solid rgba(255,0,127,0.2);border-radius:20px;padding:24px;margin-bottom:20px;box-shadow:0 0 30px rgba(255,0,127,0.08);">
            <div id="prof-photo-area" style="flex-shrink:0;">${photoHtml}</div>
            <div style="flex:1;min-width:180px;">
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                    <h2 style="font-size:1.4rem;font-weight:700;margin:0 0 6px;background:linear-gradient(135deg,#ff007f,#00f0ff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;">${escapeHtml(instructor.name)}</h2>
                    ${masteredCount > 0 ? `<span title="${lang === 'tr' ? masteredCount + ' video ustalaşıldı' : masteredCount + ' videos mastered'}" style="display:inline-flex;align-items:center;gap:4px;background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.35);border-radius:20px;padding:2px 10px;font-size:0.75rem;color:#f59e0b;font-weight:600;margin-bottom:6px;">⭐ ${masteredCount}</span>` : ''}
                </div>
                ${socialLinksHtml}
                <p id="prof-bio-display" style="font-size:0.85rem;color:#94a3b8;margin:${socialLinksHtml ? '8px' : '0'} 0 0;line-height:1.6;${!instructor.bio ? 'font-style:italic;color:#475569;' : ''}">${escapeHtml(instructor.bio) || T.noBio}</p>
            </div>
            <div style="flex-shrink:0;align-self:flex-start;">
                <button id="prof-edit-toggle-btn" class="prof-edit-btn" style="background:rgba(255,0,127,0.12);border:1px solid rgba(255,0,127,0.4);color:#ff6fae;">${T.editBtn}</button>
            </div>
        </div>

        <div id="prof-edit-form" style="display:none;background:rgba(0,240,255,0.04);border:1px solid rgba(0,240,255,0.2);border-radius:16px;padding:20px;margin-bottom:20px;">
            ...
            <div style="margin-bottom:14px;">
                <label style="font-size:0.75rem;color:#e1306c;display:block;margin-bottom:6px;"><svg style="vertical-align:middle;margin-right:4px;" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>${T.instagramLabel}</label>
                <input id="prof-instagram-input" type="url" class="prof-edit-field" placeholder="${T.igPlaceholder}" value="${escapeHtml(instructor.instagram_url || '')}">
            </div>
            <div style="margin-bottom:14px;">
                <label style="font-size:0.75rem;color:#1877f2;display:block;margin-bottom:6px;"><svg style="vertical-align:middle;margin-right:4px;" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>${T.facebookLabel}</label>
                <input id="prof-facebook-input" type="url" class="prof-edit-field" placeholder="${T.fbPlaceholder}" value="${escapeHtml(instructor.facebook_url || '')}">
            </div>
            <div style="margin-bottom:14px;">
                <label style="font-size:0.75rem;color:#ff0000;display:block;margin-bottom:6px;"><svg style="vertical-align:middle;margin-right:4px;" width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>${T.youtubeLabel}</label>
                <input id="prof-youtube-input" type="url" class="prof-edit-field" placeholder="${T.ytPlaceholder}" value="${escapeHtml(instructor.youtube_url || '')}">
            </div>
            <div style="margin-bottom:16px;">
                <label style="font-size:0.75rem;color:#94a3b8;display:block;margin-bottom:6px;">${T.bioLabel}</label>
                <textarea id="prof-bio-input" class="prof-edit-field" rows="3" placeholder="${T.bioPlaceholder}" style="resize:vertical;">${escapeHtml(instructor.bio || '')}</textarea>
            </div>
            <div style="display:flex;gap:10px;">
                <button id="prof-save-btn" class="prof-edit-btn" style="background:linear-gradient(135deg,#ff007f,#c026d3);border:none;color:#fff;">${T.saveBtn}</button>
                <button id="prof-cancel-btn" class="prof-edit-btn" style="background:transparent;border:1px solid rgba(255,255,255,0.12);color:#94a3b8;">${T.cancelBtn}</button>
            </div>
        ...
        ${activePlatforms ? `<div style="margin-bottom:20px;"><div class="prof-section-title">${lang === 'tr' ? 'Platform' : 'Platform'}</div><div>${activePlatforms}</div></div>` : ''}
        ${topTags.length > 0 ? `<div style="margin-bottom:24px;"><div class="prof-section-title">${T.topTagsTitle}</div><div>${tagsHtml}</div></div>` : ''}
    ...
    document.getElementById('prof-save-btn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            if (idx !== -1) {
                instructors[idx] = { ...instructors[idx], photo_url: photoVal || null, bio: bioVal || null, instagram_url: igVal || null, facebook_url: fbVal || null, youtube_url: ytVal || null };
    ...
    document.querySelectorAll('.prof-watch-btn[data-watch]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (platform === 'drive')        openVideoModal(vid.drive_url || vid.url);
            else if (platform === 'youtube') openVideoModal(convertYoutubeUrlToEmbed(vid.url));
            else if (vid.url)                window.open(vid.url, '_blank');
        });
    });
}